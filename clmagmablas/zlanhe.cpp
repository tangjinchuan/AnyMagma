    ...
    magma_queue_t queue )
{
    ...
    if ( uplo == MagmaLower) {
        kernel = g_runtime.get_kernel( "zlanhe_inf_kernel_lower" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
        ...
        kernel = g_runtime.get_kernel( "zlanhe_inf_kernel_upper" );
        if ( kernel != NULL ) {
            err = 0;
            arg = 0;
            err |= clSetKernelArg( kernel, arg++, sizeof(n           ), &n            );
    ...
    magma_queue_t queue )
{
    ...
    double res = magmablas_dmax_nan( n, dwork, dwork_offset, queue );
    return res;
}

...
// 单精度版本（原有功能，内核名改为 empty_kernel_float）
extern "C" void
magmablas_empty_s(
    magmaFloat_ptr dA,
    magmaFloat_ptr dB,
    ...
    magma_queue_t queue )
{
    ...
    size_t grid[1] = { magma_ceildiv(n, BLOCK_SIZE) * BLOCK_SIZE };
    
    kernel = g_runtime.get_kernel( "empty_kernel_float" );
    if ( kernel != NULL ) {
        err = 0;
        arg = 0;
    ...
    magma_queue_t queue )
{
    ...
    kernel = g_runtime.get_kernel( "empty_kernel_double" );
    if ( kernel != NULL ) {
        err = 0;
        arg = 0;
        err  = clSetKernelArg( kernel, arg++, sizeof(i0), &i0 );
